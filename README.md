...
The core domain is the **shift** (*Einsatz*), which moves through a defined state machine from planning to completion. Each role has a distinct responsibility in the process.

### Roles

| Role | Responsibility |
|---|---|
| **Admin** | User approval, role assignment, system oversight |
| **Coordinator** | Prepares shifts, assigns an initial Leader, supervises the overall process. Can also act as Leader on a shift themselves. |
| **Festmitarbeiter** | Can be assigned as Leader or team Member. As Leader: builds and manages the team for their shift. As Member: accepts or declines their invitation. |
| **Honorarkraft** | Same as Festmitarbeiter in terms of shift participation. |

### State Machine

```
Draft
  |-- (Leader proposes) --> PendingApproval
                               |-- (all members accept) --> Planned (auto)
                               |-- Coordinator start --> Active
                                                          |-- complete --> Completed
                                                          |-- cancel  --> Cancelled
```

Any status can transition to `Cancelled` by the Coordinator or assigned Leader.
