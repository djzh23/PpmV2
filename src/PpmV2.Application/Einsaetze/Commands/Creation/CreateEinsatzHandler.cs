...
public sealed class CreateEinsatzHandler
{
    private readonly IEinsatzRepository _repo;

    public CreateEinsatzHandler(IEinsatzRepository repo) => _repo = repo;

    public async Task<Guid> Handle(CreateEinsatzCommand cmd, CancellationToken ct)
    {
        ...
        else
        {
            ...
            if (cmd.Participants.Count(p => p.Role == EinsatzRole.Leader) != 1)
        ...
        var einsatz = new Einsatz
        {
            ...
            Status = EinsatzStatus.Draft
        };

        var participants = cmd.Participants!.Select(p => new EinsatzParticipant
        {
            EinsatzId = einsatzId,
            UserId = p.UserId,
            Role = p.Role
